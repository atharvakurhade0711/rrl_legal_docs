...
def test(test_dataloader, model, output_file_path, tokenizer):
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            # Append batch inputs, predictions, and true labels to the overall lists
            input_texts.extend([tokenizer.decode(input_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                                for input_ids in inputs["input_ids"]])
            predictions.extend(batch_predictions)
            true_labels.extend([tokenizer.decode(input_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                                for input_ids in batch_targets["input_ids"]])

            # Print the current batch predictions
            print("Batch Predictions:", batch_predictions)

    # Write inputs, predictions, and true labels to file
    write_predictions_to_file(output_file_path, input_texts, predictions)

    # Calculate accuracy
    wer = load("wer")
    bleu = load("bleu")
    wer_score = wer.compute(predictions=predictions, references=true_labels)
    bleu_score = bleu.compute(predictions=predictions, references=true_labels)
    
    with open('accuracy_ner_rrl_combined.txt', 'w') as f:
        f.write(f"WER Score: {wer_score} BLEU Score: {bleu_score}")
